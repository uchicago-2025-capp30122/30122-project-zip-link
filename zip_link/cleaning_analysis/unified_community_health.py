...
if __name__ == "__main__":
    pdf_file_path = 'community_health/HealthCentre1.pdf'
    csv_file_path = 'community_health/healthcentre_pdf.csv'
...
def join_health_df():
    pdf_data = process_health_data("community_health/healthcentre_pdf.csv")
    hrsa_data = get_hrsa_data("community_health/HRSA_Data.csv")
    ...
    cleaned_data.to_csv("unified_community_health_data.csv", index=False)
    zip_counts = cleaned_data["Zip Code"].value_counts().reset_index()
    zip_counts.columns = ["Zip Code", "cnt_comm_health_ctr"]
    zip_counts.to_csv("unified_community_health_count.csv", index=False)
